...
#
ANONY = [] # OFF
bot = telebot.TeleBot("5530584339:AAGxTRcDPViYhNP-2R4yospKmjodLSjUNe0M")
				...
				if men.text == '/vizinhos':
					...
					if "itemMoradores" in envia:
						try:
							...
							bot.reply_to(men, '<b>' '🔍CONSULTA DE VIZINHOS 🔍' '</b>' + '\n\n' + '<b>' '• VIZINHOS: ' '</b>' + '\n\n' + '<code>' + viz1 + '</code>' + '\n\n' + '<b>' 'Informações By: @itachivendas' '</b>' , parse_mode='html')
						except:
							try:
								viz1 = str(envia.split("<div class='itemMoradores'>")[1].split("<")[0][3:40]) + '\n' + \
									   str(envia.split("<div class='itemMoradores'>")[2].split("<")[0][3:40])
								...
								bot.reply_to(men,
											 '<b>' '🔍CONSULTA DE VIZINHOS 🔍' '</b>' + '\n\n' + '<b>' '• VIZINHOS: ' '</b>' + '\n\n' + '<code>' + viz1 + '</code>' + '\n\n' + '<b>' '𝙄𝙣𝙛𝙤𝙧𝙢𝙖𝙘̧𝙤̃𝙚𝙨 𝙗𝙮: @itachivendas' '</b>',
											 parse_mode='html')
							except:
